...
8. **⚠️ حماية تاقات UE RichText — `engine/ue_richtext.py`** (الفلتر العام لا يصلح):
   ...
   - **الحل (v2.5 — واعٍ بالأزواج)**: `ue_richtext.protect()` يستبدل كل تاق بتوكن مُرقَّم،
     لكن **بوعي بالأزواج** (مثل Bulletproof): فتح `<tag>`→`⟦N⟧` (+مكدّس)، إغلاق `</>`→`⟦/K⟧`
     (رقم فتحه المطابق)، ذاتي/قالب→`⟦sN⟧`. المودل يرى `⟦2⟧spoil⟦/2⟧` **زوجاً واضحاً** فلا
     يُسقط الفتح. (الترقيم المسطّح القديم `⟦0⟧⟦1⟧⟦2⟧⟦3⟧` كان يجعل المودل يُسقط الفتح لأنه لا
     يعرف أنه مرتبط بإغلاق.) `is_valid(out, toks, closes)` يتحقّق أن كل فتح/ذاتي + كل إغلاق
     موجود مرّة، وإلا None (لا يحفظ مكسوراً).
   - **متسامح مع تشويه المودل**: `restore`/`is_valid` يقبلان `⟦/2⟧`/`⟦2/⟧`/`⟦ 2 ⟧` (المودل
     يضيف `/` أو مسافات) → يُستعاد صحيحاً. الحذف الحقيقي ما زال يُرفَض (يعيد المحاولة بحرارة).
   - **منظّف عرض حتمي `sanitize_richtext`**: يحذف `</>` اليتيمة (تظهر **حرفياً**)، يغلق المفتوح
     المتبقّي، يضيف مسافة بعد `</>`/`<…/>` (مع استثناء الترقيم). يُطبَّق في `ManorLordsMod.build`
     على كل ترجمة + في `RetranslateWorker` — **يصلح عرض الإدخالات القديمة بلا إعادة ترجمة**.
   - **تحجيم ديناميكي + تنويع حرارة** للنصوص الطويلة كما قبل.
   - **مستخدَم في**: `build_all.py`/`translate_dt.py` + `cache.py::RetranslateWorker`. ⚠ **مزلق
     حُلَّ**: زر «إعادة الترجمة» كان عند فشل `ue.translate` (None) **لا يحدّث الكاش** فيبقى النص
     القديم المكسور (يبدو «الإصلاح لم يُطبَّق»). الآن **fallback لـ `FilteredTranslator` بوضع
     bulletproof** (نفس الترجمة الفورية المُثبَتة) ثم `sanitize_richtext`.
   - **أدوات مراجعة في صفحة الكاش**: زر «🏷 تفاصيل التاقات» في حوار التعديل (جدول يقابل كل
     تاق أصلي بمخرجه: `<h>Food</> = <h>طعام</>`) + فلتر سريع «🏷 تاقات ناقصة» (كل النصوص ذات
     عدم تطابق التاقات — `has_tag_mismatch` عبر `tag_units`/`tag_name_of` في `cache.py`) + أيقونة
     🏷 برتقالية في عمود الحالة.
...
| **حماية تاقات UE RichText (واعية بالأزواج)** | `engine/ue_richtext.py::translate/protect/sanitize_richtext` |
| **مراجعة التاقات (جدول/فلتر)** | `cache.py`: زر «🏷 تفاصيل التاقات» + فلتر «🏷 تاقات ناقصة» (`has_tag_mismatch`) |
